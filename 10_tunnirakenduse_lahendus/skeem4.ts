...
class SeriesConnection extends AbstractResistor {
    ...
    draw(g: any, startx: number, y: number) {
        g.clearRect(startx, y - this.getHeight() / 2, this.getWidth(), this.getHeight())
        let x = startx;
        g.beginPath();
        g.moveTo(x, y);
        x += 5;
        ...
        g.lineTo(x, y);
        g.stroke()
        g.fillText(this.getDataString(), startx + this.getWidth() / 2 - 10, y + this.getHeight() / 2);
    }
    getR(): number {
        let sum = 0;
